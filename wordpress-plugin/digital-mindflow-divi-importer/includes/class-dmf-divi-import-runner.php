...
class DMF_Divi_Import_Runner {
	...
	public function is_acf_ready() {
		return function_exists( 'acf_add_local_field_group' );
	}

	public function ensure_portfolio_field_groups_registered( array $args = [] ) {
		if ( ! $this->is_acf_ready() ) {
			return [];
		}

		$definitions = $this->load_portfolio_field_group_definitions();
		$registered  = [];

		foreach ( $definitions as $field_group ) {
			if ( ! is_array( $field_group ) || empty( $field_group['key'] ) ) {
				continue;
			}

			acf_add_local_field_group( $field_group );
			$registered[] = (string) ( $field_group['title'] ?? $field_group['key'] );
		}

		if ( ! empty( $args['log'] ) && ! empty( $registered ) ) {
			$this->log(
				'info',
				'Portfolio ACF field groups registered from plugin.',
				[
					'groups' => $registered,
					'path'   => $this->get_portfolio_fields_path(),
				]
			);
		}
	...
	public function apply_portfolio_case_study_enhancements( array $args = [] ) {
		$this->warnings = [];
		$dry_run        = ! empty( $args['dry_run'] );
		$this->log(
			'info',
			'Portfolio case study enhancements started.',
			[
				'dry_run' => $dry_run,
			]
		);
		...
		if ( ! $this->is_acf_ready() ) {
			$this->warn( 'Advanced Custom Fields is not active, so the bundled portfolio field group could not be registered from the plugin.' );
		} else {
			$registered_groups = $this->ensure_portfolio_field_groups_registered(
				[
					'log' => true,
				]
			);
			...
			if ( empty( $registered_groups ) ) {
				$this->warn(
					sprintf(
						'Could not load the bundled portfolio field group definition at %s.',
						$this->get_portfolio_fields_path()
					)
				);
			} else {
				$summary['acf_updated'] = array_map(
					static function ( $group_title ) {
						return sprintf( '%s (loaded from plugin)', (string) $group_title );
					},
					$registered_groups
				);
			}
		...
		$summary['portfolio_loops_updated'] = $portfolio_fix_summary['updated'];
		$summary['portfolio_loops_missing'] = $portfolio_fix_summary['missing'];
		$summary['theme_updated']           = $this->upsert_portfolio_single_theme_template( $dry_run )['updated'] ?? [];
	...
	private function apply_portfolio_loop_fix( array $args = [] ) {
		...
		if ( ! empty( $args['targets'] ) && is_array( $args['targets'] ) ) {
			$requested_targets = array_values(
				array_filter(
					array_map(
						static function ( $target ) {
							return sanitize_key( (string) $target );
						},
						$args['targets']
					)
				)
		...
		if ( ! empty( $requested_targets ) ) {
			$targets = array_values(
				array_filter(
					$targets,
					static function ( $target ) use ( $requested_targets ) {
						return in_array( sanitize_key( (string) $target['context'] ), $requested_targets, true ) ||
							in_array( sanitize_key( (string) $target['label'] ), $requested_targets, true );
					}
				)
			);
		}
	...
	private function load_portfolio_field_group_definitions() {
		$path = $this->get_portfolio_fields_path();
		...
		if ( JSON_ERROR_NONE !== json_last_error() ) {
			throw new RuntimeException(
				sprintf(
					'The bundled portfolio field group file could not be decoded: %s',
					json_last_error_msg()
				)
			);
		}
	...
	private function build_portfolio_hero_section() {
		$markup = <<<'HTML'
<div style="background:var(--gcid-dmf-card, #EDECED);width:100vw;margin-left:calc(50% - 50vw);margin-right:calc(50% - 50vw);padding:clamp(6rem, 10vw, 7.5rem) 1.5rem clamp(4rem, 8vw, 5.5rem);text-align:center">
	<div style="max-width:64rem;margin:0 auto">
		<div style="font-family:var(--gvid-dmf-body-font);font-size:var(--gvid-dmf-text-xs);font-weight:700;letter-spacing:0.22em;text-transform:uppercase;color:var(--gcid-dmf-primary, #2b5b5b);margin-bottom:calc(var(--gvid-dmf-space-xs) + 0.125rem)">Our Work</div>
		<h1 style="font-family:var(--gvid-dmf-heading-font);font-size:clamp(2.5rem, 6vw, 4.25rem);font-weight:700;line-height:1.1;color:var(--gcid-dmf-foreground, #131b26);margin:0 0 1.125rem 0">Recent <span style="display:inline-block;color:var(--gcid-dmf-accent, #941213)">Projects</span></h1>
		<p style="font-family:var(--gvid-dmf-body-font);font-size:clamp(1.0575rem, calc(1.0575rem + 0.24vw), 1.2375rem);line-height:1.8;color:var(--gcid-dmf-muted, #486262);max-width:42rem;margin:0 auto">Explore our portfolio of successful campaigns and projects. Each case study showcases our strategic approach and measurable results.</p>
	</div>
</div>
HTML;
...
.dmf-blog-loop-item{min-width:0!important;width:100%!important;max-width:none!important;flex:none!important;align-self:stretch!important}
...
.dmf-blog-loop-container--home,.dmf-blog-loop-container--archive{grid-template-columns:repeat(2,minmax(0,1fr))!important}
}
@media (max-width:767px){
.dmf-blog-loop-container--home,.dmf-blog-loop-container--archive{grid-template-columns:minmax(0,1fr)!important}
...
.dmf-home-hero-section{position:relative;overflow:hidden;isolation:isolate;padding:clamp(8rem,12vw,11rem) 0 clamp(5.5rem,8vw,7rem);background:linear-gradient(180deg,color-mix(in srgb,var(--gcid-dmf-white,#fafafa) 82%,var(--gcid-dmf-background,#fafafa)) 0%,color-mix(in srgb,var(--gcid-dmf-card,#edeced) 46%,var(--gcid-dmf-background,#fafafa)) 100%)}
.dmf-home-hero-section::before{content:"";position:absolute;left:max(-4rem,calc(50% - 42rem));top:clamp(5rem,10vw,8rem);width:clamp(14rem,24vw,21rem);height:clamp(14rem,24vw,21rem);border-radius:999px;background:radial-gradient(circle,color-mix(in srgb,var(--gcid-dmf-accent,#941213) 10%,transparent) 0%,transparent 72%);pointer-events:none}
...
.dmf-about-split{align-items:center;justify-content:space-between;gap:clamp(2.5rem,5vw,4rem)}
.dmf-about-media{flex:1 1 32rem;min-width:min(100%,22rem)}
.dmf-about-copy{flex:1 1 28rem;min-width:min(100%,20rem);max-width:38rem}
.dmf-about-copy .dmf-section-body{max-width:34rem}
.dmf-about-values{margin-top:clamp(.5rem,1.5vw,1rem)}
...
.dmf-home-actions{display:flex;flex-wrap:wrap;justify-content:center;gap:1rem;margin-top:.25rem}
...
.dmf-hero-action--primary{color:var(--gcid-dmf-white,#fafafa);background:linear-gradient(135deg,var(--gcid-dmf-accent,#941213),var(--gcid-dmf-accent-deep,#893637));border-color:color-mix(in srgb,var(--gcid-dmf-accent,#941213) 74%,var(--gcid-dmf-foreground,#131b26));box-shadow:0 .95rem 1.9rem color-mix(in srgb,var(--gcid-dmf-accent,#941213) 20%,transparent)}
.dmf-hero-action--primary:hover{color:var(--gcid-dmf-white,#fafafa);background:linear-gradient(135deg,color-mix(in srgb,var(--gcid-dmf-accent,#941213) 92%,var(--gcid-dmf-foreground,#131b26)),color-mix(in srgb,var(--gcid-dmf-accent-deep,#893637) 92%,var(--gcid-dmf-foreground,#131b26)));border-color:color-mix(in srgb,var(--gcid-dmf-accent,#941213) 84%,var(--gcid-dmf-foreground,#131b26));box-shadow:0 1.05rem 2.05rem color-mix(in srgb,var(--gcid-dmf-accent,#941213) 26%,transparent)}
.dmf-hero-action--secondary{color:var(--gcid-dmf-foreground,#131b26);background:color-mix(in srgb,var(--gcid-dmf-white,#fafafa) 82%,transparent);border-color:color-mix(in srgb,var(--gcid-dmf-border,#a1a5a4) 86%,transparent);box-shadow:0 .75rem 1.6rem color-mix(in srgb,var(--gcid-dmf-foreground,#131b26) 8%,transparent)}
.dmf-hero-action--secondary:hover{color:var(--gcid-dmf-foreground,#131b26);background:var(--gcid-dmf-white,#fafafa);border-color:color-mix(in srgb,var(--gcid-dmf-primary,#2b5b5b) 20%,var(--gcid-dmf-border,#a1a5a4));box-shadow:0 1rem 1.95rem color-mix(in srgb,var(--gcid-dmf-foreground,#131b26) 12%,transparent)}
...
.dmf-hero-scroll span{display:block;width:1.5rem;height:2.5rem;border:2px solid color-mix(in srgb,var(--gcid-dmf-border,#a1a5a4) 82%,transparent);border-radius:999px;position:relative}
...
.dmf-flex-cards{display:grid;gap:1.5rem;align-items:stretch}
.dmf-flex-cards>.et_pb_module_inner{display:contents}
.dmf-flex-cards--three{grid-template-columns:repeat(3,minmax(0,1fr))}
.dmf-flex-cards--three .dmf-lift-card,.dmf-flex-cards--three .dmf-process-step{min-width:0;align-self:stretch}
.dmf-lift-card,.dmf-process-step{display:flex;flex-direction:column;width:100%;height:100%}
.dmf-lift-card>.et_pb_module_inner,.dmf-process-step>.et_pb_module_inner{display:flex;flex-direction:column;flex:1 1 auto;height:100%}
.dmf-lift-card{gap:1rem;padding:2rem;border:1px solid var(--gcid-dmf-border,#a1a5a4);border-radius:1.35rem;background:var(--gcid-dmf-background,#fafafa);box-shadow:0 1rem 2.25rem color-mix(in srgb,var(--gcid-dmf-primary,#2b5b5b) 8%,transparent);transition:transform .28s ease,box-shadow .28s ease,border-color .28s ease}
...
.dmf-process-steps{position:relative;align-items:start;gap:clamp(2rem,4vw,3.25rem);margin-top:clamp(.5rem,1vw,.9rem)}
.dmf-process-step{position:relative;z-index:1;display:flex;flex-direction:column;align-items:center;text-align:center;gap:1rem;padding:clamp(.25rem,.75vw,.5rem) 0}
...
@media (max-width: 980px){.dmf-flex-cards--three{grid-template-columns:repeat(2,minmax(0,1fr))}.dmf-process-steps{gap:2.25rem 1.75rem}.dmf-process-step .dmf-card-copy{max-width:20rem}}
@media (max-width: 767px){.dmf-home-hero-section{padding-top:6.5rem;padding-bottom:4.75rem}.dmf-home-hero-stack{min-height:clamp(26rem,58vh,32rem);width:100%}.dmf-home-shell-row{padding:0 1rem!important}.dmf-hero-title{max-width:12ch}.dmf-mobile-single-column{display:flex!important;flex-direction:column!important}.dmf-mobile-single-column>.et_pb_column{float:none!important;width:100%!important;max-width:none!important;margin-right:0!important;margin-left:0!important}.dmf-flex-cards--three{grid-template-columns:minmax(0,1fr)}.dmf-process-steps{gap:2rem}.dmf-process-step>.et_pb_module_inner{gap:.9rem}.dmf-process-step-head,.dmf-process-step-head>.et_pb_module_inner{gap:.7rem}.dmf-process-number{width:3.65rem;min-width:3.65rem;height:3.65rem}.dmf-process-icon-frame{width:2.7rem;height:2.7rem}.dmf-process-step .dmf-card-title{max-width:none}.dmf-process-step .dmf-card-copy{max-width:24rem}.dmf-contact-panel--info,.dmf-contact-panel--form{flex-basis:100%}.dmf-home-actions{width:100%}.dmf-hero-action,.dmf-button,.dmf-button .et_pb_button,.dmf-button a.et_pb_button,.dmf-form-submit,.dmf-contact-form-shell .ff-btn-submit,.dmf-contact-form-shell button[type="submit"],.dmf-contact-form-shell input[type="submit"]{width:100%!important}}
</style>
HTML;
	}
...
.dmf-portfolio-loop-shell--portfolio .dmf-portfolio-card-media{position:relative!important;display:flex!important;flex:0 0 auto!important;margin:-1rem -1rem 0!important;width:calc(100% + 2rem)!important;overflow:hidden!important;border-radius:calc(var(--gvid-dmf-radius-lg) - 0.0625rem) calc(var(--gvid-dmf-radius-lg) - 0.0625rem) 0 0!important}
.dmf-portfolio-loop-shell--portfolio .dmf-portfolio-card-image{position:relative!important;display:flex!important;flex:1 1 auto!important;height:auto!important;min-height:0!important;aspect-ratio:16/10!important;overflow:hidden!important;border-radius:0!important;background:var(--gcid-dmf-primary,#2b5b5b)!important}
...
@media (max-width: 980px){.dmf-about-values{grid-template-columns:repeat(2,minmax(0,1fr))}}
...
HTML;
						...
						$this->build_column_module(
							...
							[
								$this->build_group_module(
									...
									[
										$this->build_portfolio_single_feature_section(
										...
										$this->build_portfolio_single_feature_section(
											...
											'Challenge',
											'project_challenge',
											'project_challenge_image',
											'left'
										),
										$this->build_portfolio_single_approach_section(),
										$this->build_portfolio_single_gallery_section(),
	...
	private function build_portfolio_single_feature_section( $admin_label, $class, $lead, $accent, $meta_key, $image_meta_key, $image_position = 'right' ) {
		$image_position_class = 'left' === sanitize_key( (string) $image_position )
			? 'dmf-portfolio-single__feature-grid dmf-portfolio-single__feature-grid--image-left'
			: 'dmf-portfolio-single__feature-grid dmf-portfolio-single__feature-grid--image-right';
				...
				$this->build_group_module(
					...
					[
						$this->build_group_module(
							...
							'dmf-portfolio-single__feature-copy'
						),
						$this->build_group_module(
							$admin_label . ' Media',
							[
								$this->build_portfolio_single_meta_image_block(
									$admin_label . ' Image',
									(string) $image_meta_key,
									'dmf-portfolio-single__image dmf-portfolio-single__image--feature'
								),
							],
							'dmf-portfolio-single__feature-media'
						),
					],
	...
	private function build_portfolio_single_gallery_section() {
		return $this->build_group_module(
			'Portfolio Single Project Gallery Section',
			[
				$this->build_portfolio_single_section_heading_block( 'Portfolio Single Project Gallery Heading', 'Project', 'Gallery' ),
				$this->build_group_module(
					'Portfolio Single Project Gallery Grid',
					[
						$this->build_portfolio_single_gallery_item_block(),
					],
					'dmf-portfolio-single__gallery-grid'
				),
			],
			'dmf-portfolio-single__section dmf-portfolio-single__section--gallery'
		);
	}
	...
	private function build_portfolio_single_gallery_item_block() {
		return $this->build_loop_group_module(
			'Portfolio Single Gallery Item',
			[
				$this->build_portfolio_single_meta_image_block(
					'Portfolio Single Gallery Image',
					'gallery_image',
					'dmf-portfolio-single__image dmf-portfolio-single__image--gallery',
					true
				),
			],
			[
				'queryType'          => 'repeater_project_gallery',
				'orderBy'            => 'date',
				'order'              => 'ascending',
				'postPerPage'        => '12',
				'postOffset'         => '0',
				'excludeCurrentPost' => 'off',
				'ignoreStickysPost'  => 'on',
				'loopId'             => 'dmfPortfolioSingleGallery',
			],
			'dmf-portfolio-single__gallery-item'
		);
	}

	private function build_portfolio_single_meta_image_block( $admin_label, $meta_key, $class = 'dmf-portfolio-single__image', $is_repeater = false ) {
		$src_token = $is_repeater
			? $this->build_portfolio_single_repeater_token( 'project_gallery', (string) $meta_key )
			: $this->build_portfolio_single_current_meta_token( (string) $meta_key );
							...
							[
								...
								'style' => $this->build_inline_style(
									[
										...
										'overflow'      => 'hidden',
										'border-radius' => '1.75rem',
										'box-shadow'    => '0 1.25rem 3rem color-mix(in srgb, var(--gcid-dmf-foreground, #131b26) 10%, transparent)',
									]
								),
							]
						),
...
.dmf-portfolio-single__metrics-grid>.entry,.dmf-portfolio-single__approach-list>.entry,.dmf-portfolio-single__gallery-grid>.entry,.dmf-portfolio-single__results-grid>.entry{display:none!important}
...
.dmf-portfolio-single__feature-grid{display:grid!important;grid-template-columns:minmax(0,1.05fr) minmax(0,.95fr)!important;align-items:center!important;gap:clamp(1.5rem,4vw,4rem)!important}
.dmf-portfolio-single__feature-copy{display:flex!important;flex-direction:column!important;gap:1rem!important}
.dmf-portfolio-single__feature-media{display:flex!important;align-items:center!important}
.dmf-portfolio-single__feature-media .dmf-portfolio-single__image,.dmf-portfolio-single__gallery-item .dmf-portfolio-single__image{width:100%!important}
.dmf-portfolio-single__feature-grid--image-left .dmf-portfolio-single__feature-copy{order:2!important}
.dmf-portfolio-single__feature-grid--image-left .dmf-portfolio-single__feature-media{order:1!important}
.dmf-portfolio-single__feature-grid:has(.dmf-portfolio-single__feature-media img[src=""]){grid-template-columns:1fr!important}
.dmf-portfolio-single__feature-grid:has(.dmf-portfolio-single__feature-media img[src=""]) .dmf-portfolio-single__feature-media{display:none!important}
.dmf-portfolio-single__image--feature img{min-height:clamp(18rem,28vw,25rem)!important;max-height:none!important}
...
.dmf-portfolio-single__gallery-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:1rem!important}
.dmf-portfolio-single__section--gallery .dmf-portfolio-single__section-title-wrap{text-align:center!important}
.dmf-portfolio-single__gallery-item{display:flex!important}
.dmf-portfolio-single__gallery-item:has(img[src=""]){display:none!important}
.dmf-portfolio-single__image--gallery img{min-height:14rem!important;max-height:none!important;aspect-ratio:16/10!important}
.dmf-portfolio-single__section--gallery:has(.dmf-portfolio-single__gallery-grid:empty),.dmf-portfolio-single__section--gallery:has(.dmf-portfolio-single__gallery-grid>.entry:only-child){display:none!important}
...
.dmf-portfolio-single__quote-box{margin-top:.5rem!important;padding:2rem 2.25rem!important;border-radius:1.5rem!important;background:#181D25!important;display:flex!important;flex-direction:column!important;gap:.65rem!important}
...
.dmf-portfolio-single__feature-grid{grid-template-columns:1fr!important}
...
.dmf-portfolio-single__metrics-grid,.dmf-portfolio-single__gallery-grid,.dmf-portfolio-single__results-grid{grid-template-columns:1fr!important}
...
.dmf-global-header-shell .dmf-global-header-column{
	...
	margin:0 !important;
	width:100% !important;
	background:color-mix(in srgb,var(--gcid-dmf-white,#fafafa) 94%,transparent) !important;
	border:1px solid color-mix(in srgb,var(--gcid-dmf-border,#a1a5a4) 72%,transparent) !important;
	border-radius:1rem !important;
	box-shadow:0 .55rem 1.6rem color-mix(in srgb,var(--gcid-dmf-foreground,#131b26) 10%,transparent) !important;
	backdrop-filter:blur(18px);
	-webkit-backdrop-filter:blur(18px);
	overflow:hidden !important;
}
.dmf-global-header-shell.dmf-header-is-scrolled .dmf-global-header-column{
	background:color-mix(in srgb,var(--gcid-dmf-white,#fafafa) 97%,transparent) !important;
	border-color:color-mix(in srgb,var(--gcid-dmf-border,#a1a5a4) 88%,transparent) !important;
	box-shadow:0 .85rem 2rem color-mix(in srgb,var(--gcid-dmf-foreground,#131b26) 12%,transparent) !important;
}
...
.dmf-global-header-shell .dmf-global-header-menu .et_pb_menu__wrap{
	...
	padding:0 clamp(1.25rem,2vw,2rem) !important;
...
.dmf-global-header-shell .dmf-global-header-menu .et-menu>li>a:hover,
.dmf-global-header-shell .dmf-global-header-menu .et-menu-nav>ul>li>a:hover,
.dmf-global-header-shell .dmf-global-header-menu .et_pb_menu__menu>nav>ul>li>a:hover,
.dmf-global-header-shell .dmf-global-header-menu .et_mobile_menu a:hover{
	color:var(--gcid-dmf-foreground,#131b26) !important;
	opacity:1 !important;
}
...
.dmf-global-header-shell .dmf-global-header-menu .et_mobile_menu{
	margin-top:.85rem !important;
	padding:.45rem 0 !important;
	background:color-mix(in srgb,var(--gcid-dmf-white,#fafafa) 98%,transparent) !important;
	border:1px solid color-mix(in srgb,var(--gcid-dmf-border,#a1a5a4) 82%,transparent) !important;
	border-radius:.85rem !important;
	box-shadow:0 .85rem 1.8rem color-mix(in srgb,var(--gcid-dmf-foreground,#131b26) 10%,transparent) !important;
}
...
@media (max-width: 980px){
	...
	.dmf-global-header-shell .dmf-global-header-menu .mobile_nav>.et_mobile_menu{
		...
		background:color-mix(in srgb,var(--gcid-dmf-white,#fafafa) 99%,transparent) !important;
		border:1px solid color-mix(in srgb,var(--gcid-dmf-border,#a1a5a4) 88%,transparent) !important;
		border-radius:1rem !important;
		box-shadow:0 1rem 2.2rem color-mix(in srgb,var(--gcid-dmf-foreground,#131b26) 12%,transparent) !important;
		z-index:1000 !important;
	}
	.dmf-global-header-shell .dmf-global-header-menu .mobile_nav.opened>.et_mobile_menu{
		display:block !important;
	}
	.dmf-global-header-shell .dmf-global-header-menu .et_mobile_menu li{
		padding:0 !important;
	}
	.dmf-global-header-shell .dmf-global-header-menu .et_mobile_menu li + li{
		border-top:1px solid color-mix(in srgb,var(--gcid-dmf-border,#a1a5a4) 42%,transparent) !important;
	}
	.dmf-global-header-shell .dmf-global-header-menu .et_mobile_menu a{
		display:block !important;
	...
	.dmf-global-header-shell .dmf-global-header-menu .mobile_menu_bar{
		...
		position:relative !important;
		cursor:pointer !important;
		border:1px solid color-mix(in srgb,var(--gcid-dmf-border,#a1a5a4) 70%,transparent) !important;
		border-radius:.8rem !important;
		background:color-mix(in srgb,var(--gcid-dmf-white,#fafafa) 95%,transparent) !important;
		box-shadow:0 .4rem 1rem color-mix(in srgb,var(--gcid-dmf-foreground,#131b26) 8%,transparent) !important;
	}
	.dmf-global-header-shell .dmf-global-header-menu .mobile_menu_bar:before{
		display:block !important;
		position:relative !important;
...
(function(){
	...
	var headerStyles={
		...
		linkTop:'var(--gcid-dmf-muted,#486262)',
		linkActiveTop:'var(--gcid-dmf-foreground,#131b26)',
	...
	try{
		debugEnabled=/(?:\?|&)dmfMenuDebug=1(?:&|$)/.test(window.location.search);
		if(!debugEnabled && window.localStorage){
			debugEnabled='1'===window.localStorage.getItem('dmfMenuDebug');
		}
	}catch(error){}
	...
	function debugLog(){
		...
		args.unshift('[DMF Menu]');
		console.log.apply(console,args);
	}
	function describeNode(node){
		if(!node){return '(null)';}
		var label=node.tagName ? node.tagName.toLowerCase() : String(node);
		if(node.id){
			label+='#'+node.id;
		}
	...
	function isMobileViewport(){
		...
		return (window.innerWidth || document.documentElement.clientWidth || 0) <= 980;
	}
	...
	function getMobileNavs(){
		return Array.prototype.slice.call(document.querySelectorAll(mobileNavSelector));
	}
	function getDirectMobileMenu(nav){
		if(!nav){return null;}
		var children=nav.children || [];
		for(var index=0; index<children.length; index+=1){
			if(children[index].classList && children[index].classList.contains('et_mobile_menu')){
				return children[index];
			}
		}
		return nav.querySelector('.et_mobile_menu');
	}
	...
	function snapshotMobileMenus(){
		return getMobileNavs().map(function(nav,index){
			...
			var toggle=getMobileToggle(nav);
			return {
				index:index,
				nav:describeNode(nav),
				toggle:describeNode(toggle),
				menu:describeNode(menu),
				opened:!!(nav && nav.classList && nav.classList.contains('opened')),
				menuDisplay:menu ? window.getComputedStyle(menu).display : null,
				menuHidden:menu ? menu.hasAttribute('hidden') : null,
				navAriaExpanded:nav ? nav.getAttribute('aria-expanded') : null,
				toggleAriaExpanded:toggle ? toggle.getAttribute('aria-expanded') : null
			};
		});
	}
	window.__dmfMenuDebug={
		enable:function(persist){
			debugEnabled=true;
			try{
				if(false!==persist && window.localStorage){
					window.localStorage.setItem('dmfMenuDebug','1');
				}
			}catch(error){}
			debugLog('debug enabled', { viewport:isMobileViewport(), width:window.innerWidth, menus:snapshotMobileMenus() });
		},
		disable:function(){
			debugEnabled=false;
			try{
				if(window.localStorage){
					window.localStorage.removeItem('dmfMenuDebug');
				}
			}catch(error){}
			if(window.console && console.log){
				console.log('[DMF Menu] debug disabled');
			}
		},
		inspect:function(){
			var snapshot=snapshotMobileMenus();
			debugLog('inspect', snapshot);
			return snapshot;
		}
	};
	...
	function prepareMobileNav(nav){
		if(!nav){return null;}
		var menu=getDirectMobileMenu(nav);
		var toggle=getMobileToggle(nav);
		if(!menu || !toggle){
			debugLog('prepareMobileNav missing parts', {
				nav:describeNode(nav),
				menu:describeNode(menu),
				toggle:describeNode(toggle)
			});
		...
		if(!menu.id){
			mobileMenuIdCounter+=1;
			menu.id='dmf-mobile-menu-'+mobileMenuIdCounter;
		}
		toggle.setAttribute('role','button');
		toggle.setAttribute('tabindex','0');
		toggle.setAttribute('aria-controls',menu.id);
		debugLog('prepareMobileNav ready', {
			nav:describeNode(nav),
			toggle:describeNode(toggle),
			menu:describeNode(menu),
			open:nav.classList.contains('opened')
		});
		return {
			menu:menu,
			toggle:toggle
	...
	function setMobileMenuState(nav,isOpen){
		if(!nav){return;}
		var prepared=prepareMobileNav(nav);
		if(!prepared){return;}
		var menu=prepared.menu;
		...
		menu.style.setProperty('display',isOpen ? 'block' : 'none','important');
		debugLog('setMobileMenuState', {
			nav:describeNode(nav),
			menu:describeNode(menu),
			isOpen:isOpen,
			display:menu.style.getPropertyValue('display'),
			hidden:menu.hasAttribute('hidden')
		});
	}
	function closeMobileMenus(exceptNav){
		getMobileNavs().forEach(function(nav){
			if(exceptNav && nav===exceptNav){return;}
			debugLog('closeMobileMenus closing', { nav:describeNode(nav), except:describeNode(exceptNav) });
			setMobileMenuState(nav,false);
		});
	}
	function toggleMobileMenu(nav){
		if(!nav){return;}
		prepareMobileNav(nav);
		var shouldOpen=!nav.classList.contains('opened');
		debugLog('toggleMobileMenu', { nav:describeNode(nav), shouldOpen:shouldOpen });
		closeMobileMenus(shouldOpen ? nav : null);
		setMobileMenuState(nav,shouldOpen);
	}
	function handleMobileToggle(event,nav){
		if(!nav || !isMobileViewport()){
			debugLog('handleMobileToggle ignored', {
	...
	function bindMobileMenuFallback(nav){
		if(!nav || nav.getAttribute('data-dmf-mobile-bound')==='true'){return;}
		var prepared=prepareMobileNav(nav);
		if(!prepared){return;}
		var toggle=prepared.toggle;
		nav.setAttribute('data-dmf-mobile-bound','true');
		debugLog('bindMobileMenuFallback', {
			nav:describeNode(nav),
			toggle:describeNode(toggle),
			menu:describeNode(prepared.menu)
		});
		setMobileMenuState(nav,false);
		function onToggle(event){
			handleMobileToggle(event,nav);
		}
		function onLinkInteraction(event){
			var target=event.target;
			var linkTarget=target && target.closest ? target.closest('.et_mobile_menu a') : null;
			if(!linkTarget){return;}
			debugLog('mobile menu link interaction', {
				target:describeNode(target),
				link:describeNode(linkTarget),
				nav:describeNode(nav)
			});
		...
		toggle.addEventListener('pointerup',onToggle,true);
		toggle.addEventListener('touchend',onToggle,{passive:false,capture:true});
		toggle.addEventListener('keydown',function(event){
			if('Enter'!==event.key && ' '!==event.key){return;}
			handleMobileToggle(event,nav);
		},true);
		toggle.onclick=onToggle;
		nav.addEventListener('click',onLinkInteraction,true);
	}
	function bindMobileMenuFallbacks(){
		debugLog('bindMobileMenuFallbacks', {
			count:getMobileNavs().length,
			isMobileViewport:isMobileViewport(),
			width:window.innerWidth
		});
		getMobileNavs().forEach(function(nav){
			prepareMobileNav(nav);
			bindMobileMenuFallback(nav);
			if(!isMobileViewport()){
				setMobileMenuState(nav,false);
			}
		});
	...
	function bindMobileMenuMutationObserver(){
		...
		menuObserver=new MutationObserver(function(){
			debugLog('menu mutation observer fired');
			bindMobileMenuFallbacks();
		});
		menuObserver.observe(document.documentElement,{childList:true,subtree:true});
		debugLog('menu mutation observer attached');
	}
	function bindMobileMenuDismiss(){
		if(document.documentElement.getAttribute('data-dmf-mobile-dismiss-bound')==='true'){return;}
		document.documentElement.setAttribute('data-dmf-mobile-dismiss-bound','true');
		...
		document.addEventListener('keydown',function(event){
			...
			debugLog('dismiss mobile menus from escape');
			closeMobileMenus();
		});
	}
...
HTML;
	...
	private function get_desired_divi_header_theme_options() {
		return [
			'divi_fixed_nav'                  => 'off',
			'boxed_layout'                    => false,
			'primary_nav_bg'                  => 'rgba(250, 250, 250, 0.96)',
			'fixed_primary_nav_bg'            => '#fafafa',
			'menu_link'                       => '#486262',
			'menu_link_active'                => '#131b26',
